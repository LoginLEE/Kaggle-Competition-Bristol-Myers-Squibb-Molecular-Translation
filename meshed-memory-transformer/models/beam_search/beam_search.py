...
class BeamSearch(object):
    ...
    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):
            ...
            #print(s.view(*([int(self.b_s), int(cur_beam_size)] + shape[1:])))
            #print( beam.expand(*([int(self.b_s), int(self.beam_size)] + shape[1:])))
            s = torch.gather(s.view(*([int(self.b_s), int(cur_beam_size)] + shape[1:])), 1,beam.expand(*([int(self.b_s), int(self.beam_size)] + shape[1:])))
            s = s.view(*([-1, ] + shape[1:]))
            return s
