...
class Transformer(CaptioningModel):
    def __init__(self, bos_idx, encoder, decoder, cnn = None, fc = None):
    ...
    def forward(self, images, seq, *args):
        if self.cnn is not None and self.fc is None:
            images = torch.unsqueeze(self.cnn(images), 1)
        elif self.cnn is not None and self.fc is not None:
            images = self.fc(self.cnn(images))
    ...
    def step(self, t, prev_output, visual, seq, mode='teacher_forcing', **kwargs):
        ...
        if mode == 'teacher_forcing':
            ...
            if t == 0:
                ...
                #     visual = torch.unsqueeze(self.cnn(visual), 1)
                if self.cnn is not None and self.fc is None:
                    visual = torch.unsqueeze(self.cnn(visual), 1)
                elif self.cnn is not None and self.fc is not None:
                    visual = self.fc(self.cnn(visual)) 
